...
export default async function LoginPage({ searchParams }: LoginPageProps) {
  ...
  return (
    // O brilho vive num irmão atrás do cartão, e não numa sombra colorida
    // dele: sombra acompanharia o retângulo, enquanto um degradê radial se
    // espalha por baixo e some antes das bordas — que é o efeito procurado.
    <div className="relative">
      <div
        aria-hidden="true"
        // Horizontalmente o brilho não passa da borda do cartão no celular:
        // com 24px para cada lado ele furava a margem de 16px e criava rolagem
        // lateral. A sangria vertical é livre — não gera scroll horizontal.
        className="pointer-events-none absolute inset-x-0 -inset-y-8 -z-10 [background:radial-gradient(55%_45%_at_50%_45%,var(--auth-glow)_0%,transparent_72%)] sm:-inset-x-6"
      />
      ...
      {/* `--card-spacing` comanda o padding e o intervalo entre cabeçalho e
          ...
          Superfície opaca: com 95% o fundo atravessava e o cartão perdia
          firmeza justamente onde o brilho passa por trás. */}
      <Card className="rounded-[1.375rem] border-border bg-card shadow-[0_36px_90px_-28px_rgba(0,0,0,0.95)] ring-1 ring-[var(--auth-glow)] [--card-spacing:--spacing(7)] sm:[--card-spacing:--spacing(8)]">
        <CardHeader>
